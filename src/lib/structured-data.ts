...
export function localBusinessJsonLd(locale: Locale = defaultLocale): JsonLdObject {
  ...
  // The Business Profile's own hours, restated for machines so the two surfaces describe
  // one business. One specification covering all seven days, because that is what the
  // Profile publishes — not seven identical rows.
  if (contact.hours) {
